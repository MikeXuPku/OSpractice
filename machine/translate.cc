...
// check for alignment errors
    ...
    //ASSERT(tlb == NULL || pageTable == NULL);  exist together	
...
// from the virtual address
	...
	entry = &pageTable[vpn];
		...
		entry = &tlb[i];	tlbhit++;		// FOUND!
		entry->time_when_use =  stats->totalTicks; //write down the time
		break;
	    }
	}
	if (entry == NULL) {				// not found
    	    ...
    	    tlbmiss++;
    	    entry = &pageTable[vpn];
	    //replace the earlest entry in TLB.
	    int time = tlb[0].time_when_use;
	    int marked = 0;
	    for(int j=0; j<TLBSize;++j){
		    if( tlb[j].time_when_use < time ){
			    time = tlb[j].time_when_use;
			    marked = j;
		    }
	    }//replace operation
	    int replaced_vpn = tlb[marked].virtualPage;
	    pageTable[replaced_vpn].time_when_use = tlb[marked].time_when_use;
	    pageTable[replaced_vpn].dirty = tlb[marked].dirty;
	    pageTable[replaced_vpn].use = tlb[marked].use;

	    tlb[marked].virtualPage = entry->virtualPage;
	    tlb[marked].physicalPage = entry->physicalPage;
	    tlb[marked].valid = true;
	    tlb[marked].readOnly = entry->readOnly;
	    tlb[marked].time_when_use = stats->totalTicks;    	    
